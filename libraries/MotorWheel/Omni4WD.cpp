...
void Omni4WD::kinematics(float vx, float vy, float omega) { //mmps
    float wheel_deg[4] = {45, 135, 225, 315};
    float wheel_cos[4];
    float wheel_sin[4];
    float omni_R = 50; //mm
	float Radius = 175; //mm
    ...
    for (int i = 0; i < 4; i++) {
        wheel_cos[i] = cos(wheel_deg[i] * 0.0174533);
        wheel_sin[i] = sin(wheel_deg[i] * 0.0174533);
    }

    float speedSP[4];
    speedSP[0] = 1000 / omni_R * (wheel_sin[0] * -vx - wheel_cos[0] * -vy + Radius * 0.001 * -omega);
    speedSP[1] = 1000 / omni_R * (wheel_sin[1] * -vx - wheel_cos[1] * -vy + Radius * 0.001 * -omega);
    speedSP[2] = 1000 / omni_R * (wheel_sin[2] * -vx + wheel_cos[2] * -vy + Radius * 0.001 * -omega);
    speedSP[3] = 1000 / omni_R * (wheel_sin[3] * -vx + wheel_cos[3] * -vy + Radius * 0.001 * -omega);

	_wheelUL->setSpeedMMPS(speedSP[0]);
	_wheelLL->setSpeedMMPS(speedSP[1]);
	_wheelUR->setSpeedMMPS(speedSP[2]);
	_wheelLR->setSpeedMMPS(speedSP[3]);
