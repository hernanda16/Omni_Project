...
def publish_euler_data(event):
    """
    Callback function for the rospy.Timer to publish Euler vector data at 100 Hz.
    """
    try:
        euler_data = bno.getVector(BNO055.VECTOR_EULER)
        msg = Float32()
        msg.data = euler_data[0]
        euler_pub.publish(msg)
    except Exception as e:
        rospy.logerr(f"Error reading Euler data: {e}")
...
if __name__ == '__main__':
    ...
    bno = BNO055()
    if not bno.begin():
        rospy.logerr("Error initializing BNO055 sensor")
        exit()

    rospy.loginfo("BNO055 sensor initialized successfully")
    # time.sleep(1)
    bno.setExternalCrystalUse(True)

    # Create a publisher for Euler data
    euler_pub = rospy.Publisher('/device/imu/yaw', Float32, queue_size=1)

    # Set up a timer to publish data at 100 Hz
    rospy.Timer(rospy.Duration(0.01), publish_euler_data)
